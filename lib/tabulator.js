...
export default class Tabulator {
  ...
  static printAsASCII(columns, rows) {
    let columnIds = Object.keys(columns);
    let data = rows.map(row => {
      let obj = {};
      for (let columnId of columnIds) {
  ...
  static printAsMarkdown(tabulatedData) {
    ...
    let tableString = '';
    // add header row
    for (let i = 0; i < tabulatedData[0].length; i++) {
      tableString += '|' + String(tabulatedData[0][i]).padEnd(columnWidths[i]);
    }
    tableString += '|\n';
    // add separator row
    for (let i = 0; i < tabulatedData[0].length; i++) {
    ...
    tableString += '|\n';
    // add data rows
    for (let j = 1; j < tabulatedData.length; j++) {
      let row = tabulatedData[j];
      for (let i = 0; i < row.length; i++) {
