...
export default class WhereAST {
  ...
  generateAst(expression) {
    if (!!expression) {
      ...
      this.verbose && console.info('WhereAST::generateAst, Tokens:', tokens);
      const ast = this.whereParser.parse(tokens);
      this.verbose && console.info('WhereAST::generateAst, AST:', JSON.stringify(ast, null, 2));
      return ast;
    } else {
  ...
  toJavaScript(ast) {
    ...
    switch (ast.type) {
      ...
      case 'NotBetweenExpression':
        return `(row[${this.toJavaScript(ast.left)}] < ${this.toJavaScript(ast.min)} || row[${this.toJavaScript(ast.left)}] > ${this.toJavaScript(ast.max)})`;
      case 'LikeExpression':
        if (this.toJavaScript(ast.right).includes('%')) { 
          let pattern = Trimmer.trim(this.toJavaScript(ast.right), '"');
          let regex = new RegExp(pattern.replace(/%/g, "\\w*"));
          return `${regex}.test(row[${this.toJavaScript(ast.left)}])`;
        } else { 
